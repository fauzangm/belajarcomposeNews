package id.android.belajarcomposenewsapps.data.repository.news
...
class NewsReposiotryImpl(
    ...
    override fun getNews(sources: List<String>,search: String): Flow<PagingData<Article>> {
        return Pager(
            config = PagingConfig(pageSize = 10),
            pagingSourceFactory = {
                NewsPagingSource(apiService = apiService, sources = search)
            }
        ).flow
    }
